...
public class GalleryFilesDTO {
	 ...
	 * 
	 * ex) gallery 1개 데이터에 파일이 3개 있는경우
	 * 1번 gallery 1번 file
	 * 1번 gallery 2번 file
	 * 1번 gallery 3번 file
	 * 
	 * 실제로는 gallery는 1개이지만 추출된 데이터는 전체가 list이므로
	 * 전체 view를 담을 DTO를 선언하고 List<DTO>형 변수에 데이터를 담았다.
	 * 
	 * 실제 데이터와 상관없이 g_seq 등등 gallery 정보를 담을 변수들은 1개만 있으면 될 것을
	 * file 개수만큼 강제로 생성하는 결과가 된다.
	 * 
	 * file의 개수만큼 gallery에 대한 정보가 쓸데없이 여러번 생성이 된다
	 */
