...
public class MemberController {
	...
	public String login(
			@RequestParam(name = "url", required = false, defaultValue = "NONE")
	...
	// HttpSession model과 비슷한 특성을 가지고 있지만 창을 종료하거나 일정시간이 지날때까지
	// 그 값을 유지하는 성질을 가지고 있다.
	...
	public String login(MemberVO memberVO, Model model, HttpSession session) {
		...
		if(memberVO == null) {
			...
			// 사용자 ID, 비밀번호 모두 정상일 경우
			// Httpsession에 사용자 정보가 담긴 memberVO 속성으로 세팅한다
			session.setAttribute("MEMBER", memberVO);
			
			/*
			 * HttpSession에 속성으로 settion된 값은 
			 * 어떠한 type이라도 상관이없다
			 * 하지만 HttpSession에 담긴 속성은 임의로 삭제하거나,
			 * 초기화하거나 서버가 멈추거나, 일정조건이 성립되지 않으면
			 * 서버 메모리에 영구히 남아있다.
			 * 
			 * 1. 가급적 작은 크기의 데이터만 담아라
			 * 2. 필요가 없어지면 반드시 소멸시켜야 한다
			 * 3. 자동 소멸되는 조건을 잘 지정하라
			 */
			// session.setAttribut("USERID", memberVO.getM_userid());
