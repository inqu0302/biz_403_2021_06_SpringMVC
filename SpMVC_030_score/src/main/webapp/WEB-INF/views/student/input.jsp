...
document.querySelector("form#student_input").addEventListener("click",(e)=>{
	...
	if(target.tagName === "BUTTON"){
		/* tag에 class를 지정할때 1개만 지정할수 있지만 다수의 class를 지정하는 경우도 있다.
		이때는 if(target.className === "className")으로 검사할 수 없다.
		이럴때는 includes를 사용하여 포함된 클래스를 검사해야 한다.
		*/
		if(target.className.includes("save")){
			// 여기에 envet 처리 코드를 작성해도 되지만 if문 내의 코드작성시 복잡하기때문에 먼저 등록한수 함수를 호출해주었다
			student_submit();
