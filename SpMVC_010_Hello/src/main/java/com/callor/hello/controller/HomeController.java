...
public class HomeController {
	...
	/*
	 * 전통적인 Java Code에서 HomeService 클래스를 객체로 선언하여 사용하는 방법
	 */
...
//	}
	...
	/* 
	 * Spring 버전 1
	 * Spring Container 에 보관된 객체중에서 HomeService 클래스를 이용하여 
	 * 생성된 객체가 있으면 나에게 달라
	 * 
	 * Spring Container 는 자신이 보관하고 있는 객체들중에 같은 이름의 객체가 있으면
	 * "주입"하여 사용할수 있도록 만들어 준다
	 * 
	 * 이 방식은 접속자가 많이 발생할 경우 메모리 leak(누수)가 발생하여 
	 * 성능이 저하되는 현상이 발생할 수 있다.
	 * 
	 * 이 방식으로 객체를 주입받으려면 객체이름을 클래스명의 첫글자만 소문자로하고
	 * 그대로 사용해야 한다.
	 */
	@Autowired
	 ...
	 * final로 또 한번 제한한다 그리고 생성자에서 매개변수로 주입을 받아
