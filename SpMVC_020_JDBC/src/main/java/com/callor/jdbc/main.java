...
public class main {
	 ...
	 * 함수에서 매개변수의 값을 변경하여도 호출하기 전 원래 변수의
	 * 값은 변함이 없다
	 * 
	 * type형 변수(Class type의 변수, class 로 생성된 객체변수)는
	 * 함수에 매개변수로 전달하였을때 변수의 요소 중 일부를 
	 * 변경하거나, 추가, 삭제하면 원래 함수의 전달하기 전의 변수값이
	 * 변경된다.
	 */
	
	public static void main(String[] args) {
		
		// 메서드 안에서 선언된 변수는 다른메서드에 의해 변경되지 않는다
		int num1 = 0;
		add(num1);
		...
		System.out.println("추가 : " + userList.toString());
	...
	// 매개변수로 받는 변수는 다른 메서드에 의해 변경이 된다
